...
ENTRYPOINT ["java","-javaagent:apm-agent.jar","-Delastic.apm.service_name=bharatpe-deposit","-Delastic.apm.server_urls=http://apm.bharatpe.in","-Delastic.apm.secret_token=gNSvBzGYqoxh","-jar","-Duser.timezone=IST","-Dspring.profiles.active=${PROFILE}", "/app/app.jar"]
