...
public class ModifyStageService {
    ...
    public void modifyLender(ModifyLenderDto modifyLenderDto) {
        ...
        if (ObjectUtils.isEmpty(existingLendingApplicationLenderDetails) || !Arrays.asList(Lender.ABFL.name(),Lender.PIRAMAL.name(), Lender.TRILLIONLOANS.name(), Lender.MUTHOOT.name(), Lender.CAPRI.name(), Lender.PAYU.name(), Lender.CREDITSAISON.name()).contains(existingLendingApplicationLenderDetails.getLender())) {
