...
public class UdyamService {
    ...
    private void triggerFetchUdyamCertificate(Long applicationId, Long merchantId, String lender){
        ...
        try {
            ...
            ResponseEntity<UdyamFetchTriggerResponse> response = restTemplate.exchange(url, HttpMethod.POST, request, UdyamFetchTriggerResponse.class);
            if(response.getStatusCode().is2xxSuccessful()){
                log.info("Udyam fetch triggered successful for merchant_id:{} and lender: {}",merchantId, lender);
            }else {
                log.error("Udyam fetch triggered unsuccessful for merchant_id:{} and lender: {}",merchantId, lender);
            }
        }catch (Exception exception){
            log.error("exception occurred in udyam fetch flow for merchant_id:{}, message: {}", merchantId, exception.getMessage());
        }
    }
