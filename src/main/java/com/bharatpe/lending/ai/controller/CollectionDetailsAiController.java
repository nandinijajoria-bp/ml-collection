...
public class CollectionDetailsAiController {
    ...
    @GetMapping(value = "/autopaydescription", produces = "application/json")
    public ResponseEntity<ApiResponse<List<List<LendingPullPaymentResponseDTO>>>> getAutopayDescription(
            @RequestAttribute(required = false) BasicDetailsDto merchant,
            @RequestParam(required = false) Long merchantId,
            @RequestParam(required = false) String date,
        ...
        log.info("Request received to get autopay description for merchantId: {}, date: {}, mode: {}", merchantId, date, mode);

        if (merchantId == null) {
            return ResponseEntity.ok(new ApiResponse<>(true, "merchantId is null"));
        }

        List<List<LendingPullPaymentResponseDTO>> allPullPaymentList =
                collectionService.getAutopayDescriptionByMerchant(merchantId, date, mode);

        String message = CollectionUtils.isEmpty(allPullPaymentList) ? "no autopay description records found" : "success";
        log.info("Fetched autopay description for merchantId: {} allPullPaymentList: {}", merchantId, allPullPaymentList);
        return ResponseEntity.ok(new ApiResponse<>(true, allPullPaymentList, message));
    }
}
