...
public interface LendingApplicationDao extends CrudRepository<LendingApplication , Long> {
	...
	List<LendingApplication> findAllByMerchantIdAndStatus(Long merchantId, String status);

	LendingApplication findByIdAndStatus(Long id, String status);

	LendingApplication findTop1ByMerchantId(Long merchantId);

	List<LendingApplication> findTop2ByMerchantIdAndPincodeOrderByIdDesc(Long merchantId, Long pincode);
...
*/
	...
	@Query(nativeQuery = true, value = "select external_loan_id from lending_application where id = :id")
	String getExternalLoanIdById(Long id);
	...
	@Query(value="select * from lending_application where merchant_id=:merchantId and status not IN ('closed','deleted') order by id desc limit 1", nativeQuery = true)
	LendingApplication findLatestOpenApplication(Long merchantId);
