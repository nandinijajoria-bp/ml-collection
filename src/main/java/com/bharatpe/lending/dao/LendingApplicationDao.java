...
public interface LendingApplicationDao extends CrudRepository<LendingApplication , Long> {
	...
	@Query(nativeQuery = true, value = "SELECT * FROM lending_application WHERE  merchant_id = :merchantId AND status NOT IN ('deleted','rejected') and NBFC_send_date is null and disburse_timestamp is null limit 1")
