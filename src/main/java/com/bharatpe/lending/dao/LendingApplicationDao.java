...
public interface LendingApplicationDao extends CrudRepository<LendingApplication , Long> {
	...
	@Query(value="select * from lending_application where merchant_id=:merchantId and status != 'deleted' and (loan_disbursal_status not in ('REJECTED') or loan_disbursal_status is null) order by id desc limit 1", nativeQuery = true)
