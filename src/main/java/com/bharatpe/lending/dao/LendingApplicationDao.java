...
public interface LendingApplicationDao extends CrudRepository<LendingApplication , Long> {
	...
	@Query(value = "select * from lending_application where loan_type='NTB' and disburse_timestamp is null and status not in ('rejected','deleted') LIMIT :offset, 1000", nativeQuery = true)
