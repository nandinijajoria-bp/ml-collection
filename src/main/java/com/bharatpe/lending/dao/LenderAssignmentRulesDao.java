...
public interface LenderAssignmentRulesDao extends JpaRepository<LenderAssignmentRules, Long> {
    ...
    @Query(value = "select * from easy_loan.lender_assignment_rules where min_bureau_score<=:bureauScore and max_bureau_score>=:bureauScore and loan_type like :loanType and risk_group like :riskGroupLike and pincode_color like :pincodeColor and is_default = false and is_active = false", nativeQuery = true)
    List<LenderAssignmentRules> fetchEligibleModelRulesForInternal( Double bureauScore, String loanType, String riskGroupLike, String pincodeColor);

    @Query(value = "select * from easy_loan.lender_assignment_rules where min_bureau_score<=:bureauScore and max_bureau_score>=:bureauScore and loan_type like :loanType and risk_group like :riskGroupLike and pincode_color like :pincodeColor and is_default = false and is_active = true", nativeQuery = true)
    List<LenderAssignmentRules> fetchEligibleModelRules(Double bureauScore, String loanType, String riskGroupLike, String pincodeColor);
