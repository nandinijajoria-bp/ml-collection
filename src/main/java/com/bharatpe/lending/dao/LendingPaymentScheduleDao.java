...
public interface LendingPaymentScheduleDao extends CrudRepository<LendingPaymentSchedule, Long> {
	...
	LendingPaymentSchedule findTop1ByMerchantIdAndStatusOrderByIdDesc(Long merchantId, String status);
	...
	@Query(
			value = "SELECT * FROM lending_payment_schedule WHERE merchant_id = :merchantId AND lms_source = :lmsSource",
			nativeQuery = true
	)
	List<LendingPaymentSchedule> findByMerchantIdAndLmsSource(Long merchantId, String lmsSource);
