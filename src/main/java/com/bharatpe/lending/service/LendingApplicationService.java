...
//                LendingApplication prevApplication = lendingApplicationDao.findTop1ByMerchantIdOrderByIdDesc(merchantBasicDetailsDto.getId());
...
//                    return createApplicationFromPrevLoan(prevApplication, requestDTO, lendingApplicationRequest.getOfferType(),merchantBasicDetailsDto);
//                } else {
...
//            lendingMerchantDropoffDao.updateApplicationId(lendingApplication.getMerchantId(), lendingApplication.getId());
//        }
...
//            lendingMerchantDropoffDao.updateApplicationId(newApplication.getMerchantId(), newApplication.getId());
...
//        for (MerchantDocumentProofSlave documentsIdProof : merchantDocumentProofs) {
//            try {
...
//
//        LendingEkyc lendingEkyc = lendingEkycDao.fetchEkycByMerchantId(documentsIdProof.getMerchantId());
//        if ("eaadhar".equalsIgnoreCase(documentsIdProof.getProofType()) && Objects.nonNull(lendingEkyc)) {
//            docKycDetails.setDocNo(lendingEkyc.getMaskedAadhar());
//        }
//
...
//    private LendingApplicationResponseDTO createApplicationFromPrevLoan(LendingApplication prevLoan, RequestDTO<LendingApplicationRequestDTO> requestDTO, String offerType, BasicDetailsDto merchantBasicDetailsDto) {
//        try {
//            return copyApplicationData(requestDTO, prevLoan, offerType, merchantBasicDetailsDto);
//        } catch (Exception e) {
...
//    private LendingApplicationResponseDTO copyApplicationData(RequestDTO<LendingApplicationRequestDTO> requestDTO, LendingApplication prevLoan, String offerType, BasicDetailsDto merchantBasicDetailsDto) {
//        try {
//            String selectedCategory = requestDTO.getPayload().getCategory();
////			if(selectedCategory==null || selectedCategory.isEmpty()) {
////				logger.error("Loan category not found in the request {}",requestDTO.toString());
...
//                return new LendingApplicationResponseDTO(false, "No eligible loan found");
//            }
////			MerchantSummary merchantSummary = merchantSummaryDao.findByMerchantId(prevLoan.getMerchantId());
//            MerchantResponseDTO merchantResponseDTO = merchantSummaryHandler.getMerchantSummary(prevLoan.getMerchantId());
//            if (ObjectUtils.isEmpty(merchantResponseDTO)) {
////				throw new RuntimeException();
//                throw new MerchantSummaryExceptionHandler(prevLoan.getMerchantId().toString());
//
//            }
//            LendingApplication newApplication = copyApplicationDataWhenExperianEnabled(eligibleLoan, prevLoan, selectedCategory,merchantBasicDetailsDto);
//            if (!StringUtils.isEmpty(requestDTO.getMeta().getLatitude()) && !requestDTO.getMeta().getLatitude().trim().equalsIgnoreCase("undefined"))
//                newApplication.setLatitude(requestDTO.getMeta().getLatitude());
//            if (!StringUtils.isEmpty(requestDTO.getMeta().getLongitude()) && !requestDTO.getMeta().getLongitude().trim().equalsIgnoreCase("undefined"))
//                newApplication.setLongitude(requestDTO.getMeta().getLongitude());
...
//            loanUtil.createApplicationSnapshot(newApplication);
//            lendingMerchantDropoffDao.updateApplicationId(newApplication.getMerchantId(), newApplication.getId());
//            signAgreementService.replicateDocumentsForNewApplication(prevLoan, newApplication, merchantBasicDetailsDto, requestDTO.getMeta());
//            return prepareAPIResponse(newApplication, true);
//        } catch (Exception e) {
...
//        int processingFee;
//        if (apiGatewayService.eligibleForProcessingFee(prevLoan.getMerchantId())) {
//            processingFee = 0;
//        } else {
...
//        newApplication.setMerchantId(prevLoan.getMerchantId());
...
//        executorService.execute(() -> apiGatewayService.globalLimitTxn(prevLoan.getMerchantId(), "DEBIT", eligibleLoan.getAmount()));
//        JsonNode smsAnalysisData = apiGatewayService.getMerchantSmsAnalysisData(merchantBasicDetailsDto);
...
//        lendingApplication.setMerchantId(merchantBasicDetailsDto.getId());
...
//        if (lendingGstDetail == null) {
//            lendingGstDetail = lendingGstDao.findTop1ByMerchantIdOrderByIdDesc(lendingApplication.getMerchantId());
//        }
...
//    }
    ...
    public Map<String, Object> getDetails(BasicDetailsDto basicDetailsDto, Long applicationId, String category, String lender) {
        ...
        if (ObjectUtils.isEmpty(merchantBankDetail)) {
            logger.error("merchantBankDetails not found for merchantId {}", basicDetailsDto.getId());
            return null;
        }
...
//        Merchant merchant = merchantDao.getById(basicDetailsDto.getId());
        try {
            ...
            if (lendingApplication != null) {
                ...
                data.put("externalLoanId", "Will be generated later");
                data.put("loanAmount", lendingApplication.getLoanAmount());
                data.put("tenure", lendingApplication.getTenure());
                data.put("interestRate", df.format(effectiveInterestRate));
                data.put("interestRateAnnum", df.format(effectiveInterestRate * 12));
                ...
                data.put("agreementAt", new SimpleDateFormat("dd-MMM-yyyy").format(new Date()));
                ...
                SimpleDateFormat date = new SimpleDateFormat("DD-MMM-YYYY");
                data.put("externalLoanId", "Will be generated later");
                data.put("loanAmount", eligibleLoan.get(0).getAmount().toString());
                data.put("tenure", tenureInMonth);
                data.put("interestRate", df.format(effectiveInterestRate));
                data.put("interestRateAnnum", df.format(effectiveInterestRate * 12));
                data.put("edi", eligibleLoan.get(0).getEdi().toString());
                data.put("lender", lender);
                data.put("mobile", basicDetailsDto.getMobile());
                data.put("location", basicDetailsDto.getCity());
                data.put("shopNumber", basicDetailsDto.getAddress());
                data.put("streetAddress", "");
                data.put("landmark", "");
                data.put("area", "");
                data.put("pincode", basicDetailsDto.getZipCode());
                data.put("city", basicDetailsDto.getCity());
                data.put("state", basicDetailsDto.getState());
                data.put("email", basicDetailsDto.getEmail());
                data.put("createdAt", new SimpleDateFormat("dd-MMM-yyyy").format(new Date()));
                data.put("agreementAt", new SimpleDateFormat("dd-MMM-yyyy").format(new Date()));
                data.put("merchantName", basicDetailsDto.getBeneficiaryName());
                data.put("payableDays", ediCount);
                data.put("businessCategory", basicDetailsDto.getBussinessCategory());
                data.put("accountNumber", merchantBankDetail.getAccountNumber());
                data.put("accountType", merchantBankDetail.getAccountType());
                data.put("ifsc", merchantBankDetail.getIfsc());
                data.put("bankName", merchantBankDetail.getBankName());
    ...
    public ResponseDTO fosnewLoan(Long merchantId) {
        ...
        try {
            ...
            if (Objects.nonNull(lendingApplicationSlave) && !"rejected".equalsIgnoreCase(lendingApplicationSlave.getStatus()) && !"SMALL_TICKET".equalsIgnoreCase(lendingApplicationSlave.getLoanType())) {
                BankDetailsDto bankDetail = merchantDetailsDto.getBankDetail();
                logger.info("Merchant bank Detais", bankDetail);
                ...
                if ("approved".equals(lendingApplicationSlave.getStatus())) {
                    ...
                    details.put("beneficiary_name", basicDetailsDto.getBeneficiaryName());
                    details.put("bank_name", bankDetail.getBankName());
                    details.put("account_number", bankDetail.getAccountNumber());
                    details.put("ifsc_code", bankDetail.getIfsc());
...
//					applicationDTO5.setComment("Call not picked");
    ...
    private boolean isCovidEffectedLoan(Long pincode, BasicDetailsDto merchant, String loanType) {
        if (pincode != null && loanUtil.isCovidCities(pincode.intValue())) {
            return true;
        }
        if (merchant.getBussinessCategory() != null && !LendingConstants.ESSENTIAL_CATEGORIES.contains(merchant.getBussinessCategory())) {
            return true;
        }
