...
public class RedisNotificationService {
	...
	public void sendNotificationForAppliedApplication(Long merchantId, LendingApplication lendingApplication) {
		try {
			...
			delayedMessagePublisher.publish("lending_notify", "notifyPartitionKey"+merchantId, notificationDto, "applied_application_30min_"+lendingApplication.getId(), 60);
			...
			delayedMessagePublisher.publish("lending_notify", "notifyPartitionKey"+merchantId, notificationDto, "applied_application_1day_"+lendingApplication.getId(), DateTimeUtil.getSecondsTillTime(13, 1));
			...
			delayedMessagePublisher.publish("lending_notify", "notifyPartitionKey"+merchantId, notificationDto,"applied_application_3day_"+lendingApplication.getId(), DateTimeUtil.getSecondsTillTime(13, 3));
			notificationDto.setMessage("Complete your Loan Application in 2 Minutes! \n" + 
					"Get Rs."+lendingApplication.getLoanAmount()+" in your "+bankName+" A/c  Now & Grow your Business.");
			//String messageString5day=objectMapper.writeValueAsString(notificationDto);
			delayedMessagePublisher.publish("lending_notify", "notifyPartitionKey"+merchantId, notificationDto, "applied_application_5day_"+lendingApplication.getId(), DateTimeUtil.getSecondsTillTime(13, 5));
		} catch (Exception e) {
	...
	public void sendNotificationForSeenOffer(Long merchantId, List<LoanEligibilityDTO> eligibleLoan) {
		try {
			...
			if(eligibleLoan!=null && !eligibleLoan.isEmpty()) {
				...
				//String messageString30min=objectMapper.writeValueAsString(notificationDto);
				delayedMessagePublisher.publish("lending_notify", "eligiblePartitionKey"+merchantId, notificationDto, "eligible_30_min_"+merchantId, 60);
				...
				//String messageString1Day=objectMapper.writeValueAsString(notificationDto);
				delayedMessagePublisher.publish("lending_notify", "eligiblePartitionKey"+merchantId, notificationDto, "eligible_1day_"+merchantId, DateTimeUtil.getSecondsTillTime(12, 1));
				...
				delayedMessagePublisher.publish("lending_notify", "eligiblePartitionKey"+merchantId, notificationDto, "eligible_3day_"+merchantId, DateTimeUtil.getSecondsTillTime(12, 3));
				//message of day 3 and day 5 are same
				delayedMessagePublisher.publish("lending_notify", "eligiblePartitionKey"+merchantId, notificationDto, "eligible_5day_"+merchantId, DateTimeUtil.getSecondsTillTime(12, 5));	
			}
		}
		catch(Exception e) {
			logger.error("Error occured while sending notification {}",e);
