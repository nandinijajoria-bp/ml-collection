...
public class RedisNotificationService {
	...
	public void sendNotificationForAppliedApplication(Long merchantId, LendingApplication lendingApplication) {
		try {
			...
			delayedMessagePublisher.publish("application_applied", "partitionKey", messageString30min, "hashKey", 30*60);
			...
			delayedMessagePublisher.publish("application_applied", "partitionKey", messageString1day, "hashKey", DateTimeUtil.getSecondsTillTime(13, 1));
			...
			delayedMessagePublisher.publish("application_applied", "partitionKey", messageString3day, "hashKey", DateTimeUtil.getSecondsTillTime(13, 3));
			notificationDto.setMessage("Complete your Loan Application in 2 Minutes! \n" + 
					"Get Rs.<Loan Amount> in your <Bank Name> A/c  Now & Grow your Business.");
			String messageString5day=objectMapper.writeValueAsString(notificationDto);
			delayedMessagePublisher.publish("application_applied", "partitionKey", messageString5day, "hashKey", DateTimeUtil.getSecondsTillTime(13, 5));
		} catch (Exception e) {
